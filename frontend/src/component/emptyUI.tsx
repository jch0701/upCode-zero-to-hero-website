...
export const EmptyUI: React.FC<EmptyBoxProps> = ({ iconSrc, title, description, classes, children }) => {
  return <Empty className={`bg-gray-800/20 rounded-2xl gap-0 p-8 sm:p-12 border border-gray-700/50 shadow-lg mr-9 ${classes || ''}`}>
    <EmptyHeader className="flex flex-col items-center gap-2 mb-6">
      ...
      <EmptyContent className="text-center">
        <EmptyTitle className="w-full text-2xl sm:text-3xl font-semibold text-white mb-2">{title}</EmptyTitle>
